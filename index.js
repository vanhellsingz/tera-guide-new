...
module.exports = function Tera_Guide(mod) {
	...
	let hooks              = [],
		...
		bossWord           = null;  // 愤怒/恐惧
	// 控制命令
	mod.command.add(["Auxiliary", "guide"], (arg) => {
		if (!arg) {
			Enabled = !Enabled;
			mod.command.message("Auxiliary tips(Guide) " + (Enabled ? "Enable(ON)" : "Disable(OFF)"));
		} else {
			switch (arg) {
				case "st":
				case "stream":
				case "主播":
					...
					mod.command.message("Anchor mode(Stream) " + (SendToStream ? "Enable(ON)" : "Disable(OFF)"));
					break;
				case "info":
				case "状态":
